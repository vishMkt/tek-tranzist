...
$(document).ready(function(){
    var currentGuard = "{{ config('auth.defaults.guard') }}";
    ...
    dataTable = $('#dataTable').DataTable({
      ...
      'ajax': {
        'headers': {
         ...
         'url': "{{ route('company.list.rides') }}".replace('company', currentGuard),
         'data': function(data){
...
function serchname(e){
    dataTable.draw(true);  
       ...
       function assignVendor(e,url,id){    
         $.ajax({
             headers: {
                 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
             },
