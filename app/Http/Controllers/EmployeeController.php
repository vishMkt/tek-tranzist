...
class EmployeeController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $qry = User::select('*')->where('users.firstname', 'LIKE', '%' . $searchValue . '%')
        ...
        foreach ($result as $row) {
            ...
            $view = route('employee.view', $row->id);
            $delete = route('employee.delete', $row->id);
    ...
    public function create()
    {
        ...
        $data['companies'] = Companie::where('status',1)->get();
    ...
    public function store(Request $request)
    {
        ...
        $user->company_id = $request->input('companie_id');
