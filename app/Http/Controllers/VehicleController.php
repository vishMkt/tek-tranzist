...
class VehicleController extends Controller
{
    ...
    { 
        ...
        return view('vehicle.create',compact('nav','drivers','vehicle_owners','makes','vendors')); 
...
public function list(Request $request)
{
    ...
    $qry = Vehicle::select('*')->orderBy('vehicles.created_at', 'DESC');

    if (isset($request->searchname)) {
        $qry->where('vehicles.license_plate', 'LIKE', '%' . $request->searchname . '%');
    }
    ...
    foreach ($result as $row) {
        ...
        $edit = route('vehicle.edit', $row->id);
        $view = route('view.vehicle', $row->id);
        $delete = route('delete.vehicle', $row->id);
        ...
        // echo "<pre>";print_r($row->ModelVehicle);die;
        $data[] = array(
            'sno'=> $i, 
            "make" => @$row->Make->name,
...
{  
    ...
    // echo "<pre>";print_r($puc->ImageFront->file_name);die;
    $models = ModelVehicle::where('make_id',$uinfo->make)->get(); 
    return view('vehicle.edit',compact('drivers','uinfo','vehicle_owners','makes','models','puc','insurance','vendors')); 
    // return response()->json(['success'=>1,'view'=>$view]);
}
...
{ 
    ...
    $vendors = Vendor::where('status',1)->get();
    $puc = Puc::where('vehicle_id',$id)->first();
    return view('vehicle.view',compact('drivers','uinfo','vehicle_owners','insurance','puc','vendors'));  
}

public function delete($id)
{ 
    Vehicle::where('id',$id)->delete();  
    Puc::where('vehicle_id',$id)->delete();  
    Insurance::where('vehicle_id',$id)->delete();  
